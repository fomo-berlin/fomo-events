...
function veventLines(ev, stamp) {
  ...
  if (tm) {
    ...
    const endHMS = tm.end || tm.start;
    lines.push(`DTSTART;TZID=${BERLIN_TZ}:${dt}T${tm.start}`);
    // if no explicit end, default +2h would need date math; keep same start->+0, use start time end fallback
    lines.push(`DTEND;TZID=${BERLIN_TZ}:${dt}T${endHMS}`);
  } else {
